...
// (`NodeFileSystem.liveContext`) — the platform test surface per the brief. They
// do real I/O under the system temp directory and clean up after themselves. The
// Node (`node:fs`) Layer Fable-compiles to JS as part of Effect.Platform.Node.
...
let private runExit (program: Effect<'A, PlatformError, Context>) : Exit<'A, PlatformError> =
    ...
    Effect.runSync NodeFileSystem.liveContext program
...
// ----------------------------------------------------------------------------
// service / layer wiring — `NodeFileSystem.layer` is what `verification` provides
// ----------------------------------------------------------------------------
...
let ``layer provides a working FileSystem`` () =
    ...
    match Effect.runSync () (Layer.provide NodeFileSystem.layer program) with
