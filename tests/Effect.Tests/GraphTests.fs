...
let ``iterates node entries in index order`` () =
    ...
    Assert.Equal<(int * string)[]>(
        ...
        Graph.toNodeEntries graph |> List.toArray
    )
...
let ``hasNode`` () =
    let graph =
        Graph.directed<string, int> (Some(fun m -> Graph.addNode m "Node A" |> ignore))
...
let ``successors and predecessors throw for undirected graphs`` () =
    ...
    Assert.Throws<GraphError>(fun () -> Graph.successors graph 0 |> ignore)
    |> ignore

    Assert.Throws<GraphError>(fun () -> Graph.predecessors graph 0 |> ignore)
    |> ignore
...
let ``neighborsDirected throws for undirected graphs`` () =
    ...
    Assert.Throws<GraphError>(fun () -> Graph.neighborsDirected graph 0 Outgoing |> ignore)
    |> ignore
...
let ``stronglyConnectedComponents throws for undirected graphs`` () =
    ...
    Assert.Throws<GraphError>(fun () -> Graph.stronglyConnectedComponents graph |> ignore)
    |> ignore
...
let ``dijkstra same source and target`` () =
    let graph =
        Graph.directed<string, int> (Some(fun m -> Graph.addNode m "A" |> ignore))
...
let ``dijkstra throws for negative weights`` () =
    ...
    let ex =
        Assert.Throws<GraphError>(fun () -> Graph.dijkstra graph 0 1 float |> ignore)
