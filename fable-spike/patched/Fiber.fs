...
module Fiber =
    ...
    // SPIKE: original reads self.Task.IsCompleted / self.Task.Result — both
    // unsupported by Fable. The fix is to track completion on the Fiber itself
    // (a `mutable Result: Exit option` cell set when the backing async finishes)
    // instead of peeking a Task. Stubbed to None here so the slice compiles; see
    // docs/fable-feasibility.md "Fiber.poll". (severity: refactor — small, local)
    let poll (self: Fiber<'A, 'E>) : Effect<Exit<'A, 'E> option, 'E2, 'R> =
        Effect.sync (fun () -> None)
