...
// ============================================================================
// SPIKE COPY of src/Effect/Effect.fs with the MINIMAL Fable-compat changes
// needed to compile to JS. Every change is tagged `// SPIKE:` with the original
// blocker. The real src/Effect/Effect.fs is UNTOUCHED. See docs/fable-feasibility.md.
// Changes: (1) unwrap drops AggregateException type-test; (2) sleep drops
// Stopwatch; (3) runSync drops Async.RunSynchronously for a non-blocking driver.
// ============================================================================
...
// SPIKE: original field is `Task: Task<Exit>` populated by Async.StartAsTask and
// awaited with Async.AwaitTask — NEITHER is implemented by Fable's runtime library
// (it ships startAsPromise/awaitPromise/startChild, not the Task variants). The
// JS-correct fiber primitive is Async.StartChild, which runs the child on the event
// loop and yields an awaitable `Async<Exit>` handle (no Task interop at all).
// (severity: refactor — a representation tweak, no architecture change)
type Fiber<'A, 'E> =
    internal
        { Join: Async<Exit<'A, 'E>>
...
module Effect =
    ...
    // SPIKE: original type-tests System.AggregateException (.InnerExceptions/.Count)
    // — unsupported by Fable, and on JS there is no AggregateException wrapping to
    // unravel. Identity is correct for the JS runtime. (severity: trivial)
    let private unwrap (ex: exn) : exn = ex
    ...
    // SPIKE: original takes `unit -> Task<'A>` and bridges with Async.AwaitTask,
    // unsupported by Fable. On JS the host yields a Promise; the real port bridges
    // it with Async.AwaitPromise (Fable.Core.JS.Promise). For the spike we accept an
    // Async thunk directly to drop the Task dependency. (severity: refactor)
    ...
    let fork (Effect run) : Effect<Fiber<'A, 'E>, 'E2, 'R> =
        Effect(fun _ r ->
            async {
                ...
                // SPIKE: was Async.StartAsTask (unsupported). Async.StartChild runs
                // the child concurrently on the event loop and returns an awaitable
                // Async handle that memoizes the result for re-await. (refactor)
                let! handle = Async.StartChild(run child r)
                return Success { Join = handle; Runtime = child }
            })
    ...
    let await (fib: Fiber<'A, 'E>) : Effect<Exit<'A, 'E>, 'E2, 'R> =
        Effect(fun _ _ ->
            async {
                let! e = fib.Join // SPIKE: was Async.AwaitTask fib.Task
                return Success e
            })

    /// Wait for a fiber and fold its failure back into this effect. (Fiber.join)
    let join (fib: Fiber<'A, 'E>) : Effect<'A, 'E, 'R> =
        Effect(fun _ _ -> async { return! fib.Join }) // SPIKE: was Async.AwaitTask fib.Task
    ...
    let interrupt (fib: Fiber<'A, 'E>) : Effect<unit, 'E2, 'R> =
        Effect(fun _ _ ->
            async {
                ...
                let! _ = fib.Join // SPIKE: was Async.AwaitTask fib.Task
                return Success()
            })
    ...
    let sleep (milliseconds: int) : Effect<unit, 'E, 'R> =
        Effect(fun fib _ ->
            async {
                // SPIKE: original uses System.Diagnostics.Stopwatch (unsupported by
                // Fable). Track remaining millis with a counter instead — same
                // cooperative poll-for-interrupt loop. (severity: trivial)
    ...
    // SPIKE: original is `runExit env eff |> Async.RunSynchronously`.
    // Async.RunSynchronously CANNOT exist on single-threaded JS — there is no way
    // to block the event loop waiting for a suspended async. This non-blocking
    // driver kicks the workflow off synchronously (Async.StartImmediate) and reads
    // the result cell: for a fully-synchronous effect it is already populated; if
    // the effect actually suspended (real await/sleep/fork-join), it raises — which
    // is exactly Effect-TS's runSync contract ("Die: synchronous effect suspended").
    // (severity: refactor — a JS-specific runner; semantics preserved for sync effects)
    let runSync (env: 'R) (eff: Effect<'A, 'E, 'R>) : Exit<'A, 'E> =
        ...
        match result with
        | Some x -> x
        | None -> failwith "runSync: effect suspended asynchronously; use runPromise/runPromiseExit on JS"
